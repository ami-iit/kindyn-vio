...
protected:
    ...
    ImuBias m_imuBiasHat;
    CDMBias m_cdmBiasHat;
    ...
    virtual gtsam::Vector getBiasCorrections(const ImuBias& imuBias_i,
                                             const CDMBias& cdmBias_i) const = 0;
...
public:
    ...
    virtual void resetIntegrationAndSetBias(const ImuBias& imuBiasHat,
                                            const CDMBias& cdmBiasHat) = 0;
    ...
    const CDMBias& centroidalDynamicsBiasHat()        const { return m_cdmBiasHat;          }
    gtsam::Vector centroidalDynamicsBiasHatVector()   const { return m_cdmBiasHat.vector(); }
    const ImuBias& imuBiasHat()                       const { return m_imuBiasHat;          }
    gtsam::Vector imuBiasHatVector()                  const { return m_imuBiasHat.vector(); }
    ...
    virtual gtsam::Vector computeErrorAndJacobians(const gtsam::Pose3& H_i,
                                                   ...
                                                   gtsam::OptionalJacobian<24, 6> H9 = boost::none,
                                                   gtsam::OptionalJacobian<Eigen::Dynamic, Eigen::Dynamic> H10 = boost::none,
                                                   gtsam::OptionalJacobian<24, 6> H11 = boost::none,
                                                   gtsam::OptionalJacobian<Eigen::Dynamic, Eigen::Dynamic> H12 = boost::none) const = 0;
};
...
protected:
    ...
    gtsam::Vector getBiasCorrections(const ImuBias& imuBias_i,
                                     const CDMBias& cdmBias_i) const override;
public:
    PreintegratedCDMCumulativeBias();
    PreintegratedCDMCumulativeBias(const std::shared_ptr<Params>& p,
                                   const ImuBias& imubiasHat = ImuBias(),
                                   const CDMBias& cdmbiasHat = CDMBias());
    ...
    void resetIntegrationAndSetBias(const ImuBias& imuBiasHat,
                                    const CDMBias& cdmBiasHat) override;
    ...
    gtsam::Vector computeErrorAndJacobians(const gtsam::Pose3& H_i,
                                           ...
                                           gtsam::OptionalJacobian<Eigen::Dynamic, Eigen::Dynamic> H10 = boost::none,
                                           gtsam::OptionalJacobian<24, 6> H11 = boost::none,
                                           gtsam::OptionalJacobian<Eigen::Dynamic, Eigen::Dynamic> H12 = boost::none) const override;
